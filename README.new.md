...
Open the preview URL shown in terminal (usually [http://localhost:4173](http://localhost:4173)).
...
- No app-level auth or full shell navigation was added because this screen is treated as an embedded module inside already-authenticated host applications.
- Internal and lender views are accessible in one shell to simplify interview demoing and reduce context-switching.
- SPA state-driven lender switching was chosen over routing for prototype speed and simplicity.

### Trade-offs

- Client-side parsing is acceptable for challenge scope but would not be ideal at higher production volumes.
...
Started the task by using Claude to identify the detail behind each header and ensuring that I had a clear understanding of the data and terminology used. I have a lot of domain knowledge to pick up about the mortgage industry although the terminology quickly became obvious to me as I read through the data.

I then wrote up my assumptions in this file from reading the task, stripping back quite a bit of content to bring the scope of the challenge down to just indicate value add for users rather than including all the typical functionality that would be expected in a real application. I collated context data such as the task information, job description, screenshots from Acre's website for styling reference and fed these to Claude to research and generate a detailed plan of the task and generate initial mock ups.
...
I double checked that the dashboard values were correct and aligned with the data in the CSV file by converting the CSV into Excel formula and analysing the data using formulas.

On Monday morning I cleaned up the README.md file and committed the final version to GitHub for submission.
...
- Multiple date/time formats are normalized during parsing to support consistent aggregation and charting.
...
To simplify funnel analysis, statuses are grouped as:
...
## Topic Summary from INFORMATION.md

### Product and UX priorities

- With additional time, user research would guide feature prioritization before implementation.
- Accessibility improvements (ARIA semantics, assistive technology support) are identified as a next-step focus area.
- Performance and polish improvements would target loading behavior, tooltips, micro-interactions, and navigation smoothness.
...
- Expanded pipeline diagnostics to identify where cases stall and why.

### Data and modeling enhancements desired

- Better upstream data completeness and quality controls for key attributes.
- Visibility of case status immediately prior to `NOT_PROCEEDING` to improve root-cause analysis.
